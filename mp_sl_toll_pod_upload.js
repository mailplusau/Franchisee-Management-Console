   ...
   * @Last modified time: 2021-11-18T09:06:06+11:00
   */
  ...
  function main(request, response) {
    if (request.getMethod() == "GET") {
      ...
      operatorID = parseInt(request.getParameter('operatorID'));
      ...
      if (operatorID == 621) {
        inlineHtml +=
          '<option value="621" selected="selected">Zhao Yang Lin (St. Kilda Road)</opion>'
      ...
      } else if (operatorID == 108) {
        inlineHtml +=
          '<option value="621">Zhao Yang Lin (St. Kilda Road)</opion>'
        inlineHtml +=
          '<option value="108" selected="selected">Vlado Nikoloski (Sunshine)</opion>'
        inlineHtml += '<option value="687">Jason Knight (Mulgrave)</opion>'
        inlineHtml += '<option value="655">David Hicks (Moorabin)</opion>'
      } else if (operatorID == 687) {
        inlineHtml +=
          '<option value="621">Zhao Yang Lin (St. Kilda Road)</opion>'
        inlineHtml += '<option value="108">Vlado Nikoloski (Sunshine)</opion>'
        inlineHtml +=
          '<option value="687" selected="selected">Jason Knight (Mulgrave)</opion>'
        inlineHtml += '<option value="655">David Hicks (Moorabin)</opion>'
      } else if (operatorID == 655) {
        inlineHtml +=
          '<option value="621">Zhao Yang Lin (St. Kilda Road)</opion>'
        inlineHtml += '<option value="108">Vlado Nikoloski (Sunshine)</opion>'
        inlineHtml += '<option value="687">Jason Knight (Mulgrave)</opion>'
        inlineHtml +=
          '<option value="655" selected="selected">David Hicks (Moorabin)</opion>'
      }
      ...
      inlineHtml += '</div>';
      inlineHtml += '</div>';
      // form.addField('upload_file_1', 'file', 'Service Commencement Form').setLayoutType('outsideabove', 'startrow').setDisplaySize(40);
      var inlineHtml2 = '<div class="form-group container zee_section">';
      inlineHtml2 += '<div class="row">';
      inlineHtml2 +=
        '<img id="viewer" frameborder="0" scrolling="no" width="400" height="600" />';
      inlineHtml2 += '</div>';
      inlineHtml2 += '</div>';

      form.addField('custpage_inlinehtml1', 'inlinehtml', '').setLayoutType(
        'startrow').setDefaultValue(inlineHtml);
      form.addField('upload_file_1', 'file', 'TOLL POD Form').setLayoutType(
        'midrow', 'startrow').setDisplaySize(40).setMandatory(true);
      form.addField('custpage_inlinehtml2', 'inlinehtml', '').setLayoutType(
        'outsidebelow').setDefaultValue(inlineHtml2);
      ...
      form.addField('custpage_json', 'text', 'Image to Text JSON').setDisplayType(
        'hidden');
      form.addField('custpage_tollpodid', 'text', 'TOLL POD ID').setDisplayType(
        'hidden').setDefaultValue(tollPODRecordID);
      ...
      if (!isNullorEmpty(file)) {
        ...
        nlapiSetRedirectURL('EXTERNAL',
          'https://1048144.extforms.netsuite.com/app/site/hosting/scriptlet.nl?script=1388&deploy=1&compid=1048144&h=718b6164fd2f1ca09435',
          null, null, params);
